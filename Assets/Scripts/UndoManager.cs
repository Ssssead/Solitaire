...
public class UndoManager : MonoBehaviour
{
    ...
    public IEnumerator UndoLastCoroutine()
    {
        ...
        // --- ДОБАВИТЬ ЭТО: Отслеживаем использование отмены ---
    ...
    private void AddCardsBackToSource(MoveRecord record, bool immediate)
    {
        ...
        if (source.GetType().Name.Contains("Stock"))
        {
            ...
            // Поддержка кастомных стоков (например, SultanStockPile)
            else
            {
        ...
        // --- 🔥 ИСПРАВЛЕНИЕ: УНИВЕРСАЛЬНЫЙ ФОЛЛБЭК ДЛЯ НОВЫХ РЕЖИМОВ 🔥 ---
        // Если UndoManager не знает этот класс (как в случае с Султаном), 
        // он просто заставит контейнер принять карту через базовый интерфейс.
        else
        {
            foreach (var card in cards)
            {
                ...
                // Вызываем родной метод контейнера
                source.AcceptCard(card);

                // Гарантируем, что карта физически покинула DragLayer
