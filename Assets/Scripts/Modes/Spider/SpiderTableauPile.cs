...
public class SpiderTableauPile : TableauPile
{
    ...
    private IEnumerator MoveSequenceToFoundation(int startIndex)
    {
        ...
        {
            // <--- ИСПРАВЛЕНИЕ БАГА: ЖЕСТКОЕ ПРЕРЫВАНИЕ ПРИ ОТМЕНЕ --->
            if (spiderMode != null && spiderMode.undoManager != null && spiderMode.undoManager.IsUndoing)
            {
                // Если игрок отменил ход прямо сейчас, отменяем и эту анимацию
                if (spiderMode != null) spiderMode.ActiveFoundationAnimations--;
                isAssembling = false;
                yield break;
            }
        ...
        // Проверяем еще раз после полета
        if (spiderMode != null && spiderMode.undoManager != null && spiderMode.undoManager.IsUndoing)
        {
            if (spiderMode != null) spiderMode.ActiveFoundationAnimations--;
            isAssembling = false;
    ...
    private IEnumerator AnimateCardToFoundation(CardController card, SpiderFoundationPile foundation, float duration)
    {
        ...
        while (t < duration)
        {
            // <--- ИСПРАВЛЕНИЕ БАГА: ПРЕРЫВАЕМ ПОЛЕТ КОНКРЕТНОЙ КАРТЫ --->
