...
public class StockPile : MonoBehaviour, ICardContainer, IPointerClickHandler
{
    ...
    /// <summary>
    /// Возвращает мировую позицию, где должна лежать карта с указанным индексом.
    /// Используется DeckManager-ом для анимации точного полета.
    /// </summary>
    public Vector3 GetWorldPositionForIndex(int index)
    {
        // 1. Рассчитываем локальное смещение так же, как в AddCard
        float xOffset = index * stackGap;

        // 2. Создаем локальный вектор (можно добавить небольшой Z, чтобы карты не мерцали)
        // Z-смещение: каждая следующая карта чуть ближе к камере (-0.1f)
        Vector3 localPos = new Vector3(xOffset, 0f, index * -0.02f);

        // 3. Переводим локальную точку в глобальные координаты
        // Это учитывает позицию, поворот и масштаб самого StockPile
