...
public static class KlondikeSolver
{
    ...
    // Контекст для передачи данных в рекурсии (чтобы не таскать кучу аргументов)
    private class SolveContext
    {
        public int DeadEndsCount = 0;    // Счетчик тупиков (наша сложность)
        public int StatesVisited = 0;    // Счетчик нагрузки
        public HashSet<string> History = new HashSet<string>(); // Защита от циклов
        public bool Solved = false;
        public int DrawParam;            // 1 или 3 карты
        public int SolutionMoves = 0;    // Длина решения
    }
    ...
    private class MoveCommand
    {
        ...
        public int ToIdx;
        public int Count;
        public int Priority; // Для сортировки (эвристика)
    }

    // =================================================================================
    // 1. ОСНОВНОЙ МЕТОД: ГЛУБОКИЙ ПОИСК (DFS) + ПОДСЧЕТ ТУПИКОВ
    // =================================================================================
    ...
    public static SolverResult Solve(Deal initialDeal, int drawCount)
    {
        ...
        if (!success)
        {
            return new SolverResult { IsSolved = false };
        }

        // Классификация
        Difficulty diff;
        int traps = ctx.DeadEndsCount;

        if (traps <= 2) diff = Difficulty.Easy;
        else if (traps <= 5) diff = Difficulty.Medium;
    ...
    private static bool DFS(Deal currentDeal, int depth, SolveContext ctx)
    {
        ...
        ctx.StatesVisited++;
        if (ctx.StatesVisited > MAX_STATES) return false; // Лимит операций
    ...
    // =================================================================================
    // 2. ВСПОМОГАТЕЛЬНЫЙ МЕТОД: ЖАДНЫЙ БОТ (GREEDY)
    // =================================================================================

    /// <summary>
    /// Проверяет, можно ли выиграть, просто делая очевидные ходы без планирования.
    /// Возвращает TRUE, если расклад тривиален.
    /// </summary>
    public static bool IsSolvableByGreedy(Deal initialDeal, int drawCount)
    {
        ...
        while (movesWithoutAction < 2 && safetyLimit > 0)
        {
            safetyLimit--;
            bool moveMade = false;

            if (TryMoveToFoundationGreedy(d))
            {
                moveMade = true; movesWithoutAction = 0; continue;
            }
            if (TryTableauMoveGreedy(d, onlyRevealing: true))
            {
                moveMade = true; movesWithoutAction = 0; continue;
            }
            if (TryWasteToTableauGreedy(d))
            {
                moveMade = true; movesWithoutAction = 0; continue;
            }
            ...
            if (d.stock.Count > 0)
            {
                DrawCards(d, drawCount);
                moveMade = true; movesWithoutAction = 0;
            }
    ...
    // =================================================================================
    // 3. ЛОГИКА ГЕНЕРАЦИИ ХОДОВ (ДЛЯ DFS)
    // =================================================================================
    ...
    private static Deal ApplyMove(Deal oldState, MoveCommand m)
    {
        ...
        switch (m.Type)
        {
            case MoveType.Foundation:
                if (m.FromIdx == -1) // Из Waste
                {
                    var c = d.waste.Last(); d.waste.RemoveAt(d.waste.Count - 1);
            ...
            case MoveType.StockDraw:
                // Для простоты симуляции тупиков берем по 1 карте.
                // Если критично Draw 3 - нужно добавить цикл.
                var x = d.stock.Pop();
                x.FaceUp = true;
                d.waste.Add(x);
                break;
    ...
    private static bool IsSignificantMove(MoveCommand m)
    {
        // Считаем ошибкой только если мы сделали "умный" ход на столе, а он оказался плохим.
        // Простое листание колоды не считается ошибкой стратегии.
        return m.Type != MoveType.StockDraw && m.Type != MoveType.RecycleWaste;
    }

    // =================================================================================
    // 4. МЕТОДЫ ДЛЯ ЖАДНОГО БОТА (In-Place Modifications)
    // =================================================================================
    // Эти методы изменяют Deal напрямую, чтобы не создавать мусор (GC) при жадном поиске.
    ...
    private static bool TryMoveToFoundationGreedy(Deal d)
    {
        for (int i = 0; i < 7; i++)
        {
            if (d.tableau[i].Count > 0)
            {
                ...
                if (c.FaceUp && CanAddToFoundation(d, c.Card))
                {
                    d.foundations[(int)c.Card.suit].Add(c.Card); d.tableau[i].RemoveAt(d.tableau[i].Count - 1);
                    if (d.tableau[i].Count > 0) d.tableau[i].Last().FaceUp = true; return true;
                }
            }
        }
        if (d.waste.Count > 0)
        {
            var c = d.waste.Last();
            if (CanAddToFoundation(d, c.Card))
            {
                d.foundations[(int)c.Card.suit].Add(c.Card); d.waste.RemoveAt(d.waste.Count - 1); return true;
            }
        }
        return false;
    }
    ...
    private static bool TryTableauMoveGreedy(Deal d, bool onlyRevealing)
    {
        ...
        {
            if (d.tableau[src].Count == 0) continue;
            var pile = d.tableau[src];
            int faceUpIdx = -1;
            for (int k = 0; k < pile.Count; k++) { if (pile[k].FaceUp) { faceUpIdx = k; break; } }
            if (faceUpIdx == -1) continue;
            bool exposesHidden = (faceUpIdx > 0 && !pile[faceUpIdx - 1].FaceUp);
            bool emptiesSlot = (faceUpIdx == 0);
            if (onlyRevealing) { if (!exposesHidden && !emptiesSlot) continue; if (emptiesSlot) continue; }
            var card = pile[faceUpIdx].Card;
            for (int dest = 0; dest < 7; dest++)
            {
                ...
                if (CanPlaceOnTableau(d, dest, card))
                {
                    var moving = pile.GetRange(faceUpIdx, pile.Count - faceUpIdx);
                    d.tableau[dest].AddRange(moving); d.tableau[src].RemoveRange(faceUpIdx, moving.Count);
                    if (d.tableau[src].Count > 0) d.tableau[src].Last().FaceUp = true; return true;
                }
            }
        }
        return false;
    ...
    {
        if (d.waste.Count == 0) return false;
        var cardInst = d.waste.Last();
        for (int i = 0; i < 7; i++)
        {
            if (CanPlaceOnTableau(d, i, cardInst.Card))
            {
                d.tableau[i].Add(cardInst); d.waste.RemoveAt(d.waste.Count - 1); return true;
            }
        }
        return false;
    }
    ...
    {
        bool redA = (a.suit == Suit.Diamonds || a.suit == Suit.Hearts);
        bool redB = (b.suit == Suit.Diamonds || b.suit == Suit.Hearts);
        return redA != redB;
    }

    private static void DrawCards(Deal d, int count)
    {
        for (int i = 0; i < count && d.stock.Count > 0; i++)
        {
            var c = d.stock.Pop();
            c.FaceUp = true;
            d.waste.Add(c);
    ...
    {
        // Важно: Собираем в обратном порядке
    ...
    {
        int fCount = 0;
        foreach (var f in d.foundations) fCount += f.Count;
        return fCount == 52;
    }

    private static string GetStateHash(Deal d)
    {
        // Уникальный ключ состояния (для DFS)
        // Хэш = Фонды + Топы стола + Размер стока/сброса
        int hash = 17;
        foreach (var f in d.foundations) hash = hash * 31 + (f.Count > 0 ? f.Last().rank : 0);
        foreach (var t in d.tableau)
        {
            if (t.Count > 0)
            {
                var c = t.Last().Card;
                hash = hash * 31 + (c.rank * 10 + (int)c.suit);
            }
            else hash = hash * 31 + -1;
        }
        return $"{hash}:{d.stock.Count}:{d.waste.Count}";
    }
}